...
async def get_balance(session, row):
    """Load"""
    ...
    try:
        async with session.get(f"http://localhost:3000/v2/{msisdn}/poin?bucket_type=TelkomselPOIN&identifier=MSISDN" ,headers = headers) as response:
            ...
            # GET BALANCE
            try:
                # balance_data = json.loads(await get_balance(session=session, msisdn=msisdn))
...
async def process_data(df):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [get_balance(session, dataset) for dataset in df.itertuples()]
